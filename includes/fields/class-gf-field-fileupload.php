...
class GF_Field_FileUpload extends GF_Field {
	...
	public function is_invalid_file( $file, $is_new = true ) {
		...
		if ( $is_new ) {
			...
			} elseif ( str_contains( rgar( $file, 'url' ), '..' ) ) {
				GFCommon::log_debug( __METHOD__ . '(): The file path contains directory traversal characters.' );
				$message = $this->errorMessage ?: esc_html__( 'This URL is not permitted.', 'gravityforms' );
